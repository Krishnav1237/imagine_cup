      ...
      <main className="flex-1 flex flex-col pt-24 px-6 max-w-5xl mx-auto w-full pb-10">
        ...
        <header className="mb-8 flex items-center justify-between">
          <div>
            ...
            {currentChunk.generation_model && (
              <p className="text-xs text-[#8888a0] mt-1">
                Generated by {currentChunk.generation_model}
              </p>
            )}
          </div>
          <div className="text-right">
             <div className="text-2xl font-bold font-mono text-[#ff6b4a]">
               {Math.floor((currentChunk.duration_seconds ?? 300) / 60)}:{((currentChunk.duration_seconds ?? 300) % 60).toString().padStart(2, '0')}
             </div>
